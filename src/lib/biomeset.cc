...
namespace mm {
  ...
  biome& biomeset::add_terrain(const biome& biome) {
...
#define TILESET_SIZE 256
  ...
  biomeset biomeset::whittaker() {
    ...
    // very high altitudes
    set.add_terrain({ "Scorched",                   { 0x55, 0x55, 0x55 }}).add_climate({ 0.9, 1 }, { 0, 0.16 }, false);
    set.add_terrain({ "Bare",                       { 0x88, 0x88, 0x88 }}).add_climate({ 0.9, 1 }, { 0.16, 0.33 }, false);
    set.add_terrain({ "Tundra",                     { 0xBB, 0xBB, 0xAA }}).add_climate({ 0.9, 1 }, { 0.33, 0.5 }, false);
    set.add_terrain({ "Snow",                       { 0xFF, 0xFF, 0xFF }}).add_climate({ 0.9, 1 }, { 0.5, 1 }, false);
